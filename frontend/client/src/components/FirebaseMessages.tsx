      ...
      <div className={`${showChat ? 'flex' : 'hidden'} md:flex flex-col flex-1 bg-slate-50`}>
        {selectedConversation ? (
          <>
            ...
            {/* Messages */}
            <div className="flex-1 overflow-y-auto px-4 sm:px-5 md:px-6 lg:px-8 py-4" style={{ backgroundColor: '#e5ddd5' }}>
              {messages.length === 0 ? (
                      ...
                      <div key={msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                        <div
                          className={`relative py-2 px-3 rounded-lg shadow-sm ${
                            isOwn
                              ? 'bg-[#dcf8c6] rounded-tr-none ml-14 sm:ml-16 md:ml-20'
                              : 'bg-white rounded-tl-none mr-14 sm:mr-16 md:mr-20'
                          }`}
